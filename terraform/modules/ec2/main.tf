###############################################################################
# EC2 Module
# Creates: Security Groups, ALB, Target Group, Launch Template, ASG,
#          and a CPU-based target-tracking scaling policy.
###############################################################################
...
resource "aws_security_group" "ec2" {
  ...
  # SSH — locked to VPC CIDR (bastion / SSM preferred in production)
  ingress {
    description = "SSH within VPC only"
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
  ...
  egress {
    description = "Allow all outbound (for Docker pulls, AWS API calls)"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
...
# Allow the ALB service to write logs to the bucket
...
resource "aws_autoscaling_group" "app" {
  ...
  lifecycle {
    create_before_destroy = true
    ignore_changes        = [desired_capacity] # Let ASG manage desired count
  }
}

# ── Target Tracking Scaling Policy (CPU) ──────────────────────────────────────
# Scales out when average CPU > 60%, scales in when it drops back below
...
# ── Step Scaling Policy: ALB Request Count ────────────────────────────────────
# Provides faster scale-out response to sudden traffic spikes
